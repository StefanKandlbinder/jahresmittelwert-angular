...
export class MeasurementService {
  ...
  getData(urls: string[], mean: string): Observable<Measurement[]> {
    return from(urls).pipe(
      concatMap((url: string) => {
        return this.http.request<Measurements>('get', url, {
          responseType: 'json',
        });
      }),
  ...
  getDataStation(
    ...
    stationData.stationShort = this.stationsService.getStationByCode(
      measurements[0].station
    ).kurzname;
    ...
    mean === 'TMW' || (mean === 'HMW' && days === 1)
      ? (stationData.value = mittelwert.toFixed(2).toString())
      : (stationData.value = (
          parseFloat(
            measurements[measurements.length - 1].messwert.replace(',', '.')
          ) * 1000
        )
          .toFixed(2)
          .toString());
